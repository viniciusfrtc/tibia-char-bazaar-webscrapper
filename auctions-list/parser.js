...
const getCharacterLevel = rawCharacterDetails => {
    const levelRawString = rawCharacterDetails.find(rawDetail => rawDetail.startsWith(LEVEL_PREFIX))
    return levelRawString.substring(LEVEL_PREFIX.length)
}

const getCharacterVocation = rawCharacterDetails => {
    const vocationRawString = rawCharacterDetails.find(rawDetail => rawDetail.startsWith(VOCATION_PREFIX))
...
const getCharacterWorld = rawCharacterDetails => {
    const worldRawString = rawCharacterDetails.find(rawDetail => rawDetail.startsWith(WORLD_PREFIX))
    return worldRawString.substring(WORLD_PREFIX.length)
}

const getCharacterDetails = rawCharacterDetails => {
    const sanitizedRawCharacterDetails = sanitizeCharacterDetails(rawCharacterDetails)
...
const getAuctionFinalStatus = rawAuctionDetails => {
    const [finalStatus] = rawAuctionDetails.find(detail => detail.endsWith(BID_SUFFIX)).split(' ')
